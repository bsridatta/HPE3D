...
def get_camera_data(camera=1, subject=1, param='r'):
    ...
    calibration_matrix: 3x3 calibration matrix [[fx,0,cx][0,fy,cy][0,0,1]]
    distortion: 5 values [k1, k2, p1, p2, k3]
    """
    param = param.lower()

    if param == 'r':
        val = CAM_PARAMAS['extrinsics'][f'S{subject}'][f'C{camera}']['R']
    elif param == 't':
        val = CAM_PARAMAS['extrinsics'][f'S{subject}'][f'C{camera}']['t']
    elif param == 'f':
        matrix = CAM_PARAMAS['intrinsics'][f'C{camera}']['calibration_matrix']
        val = [matrix[0][0], matrix[1][1]]
    elif param == 'c':
        matrix = CAM_PARAMAS['intrinsics'][f'C{camera}']['calibration_matrix']
        val = [matrix[0][2], matrix[1][2]]
    elif param == 'k':
        distortion = CAM_PARAMAS['intrinsics'][f'C{camera}']['distortion']
        val = [distortion[0], distortion[1], distortion[4]]
    elif param == 'p':
        distortion = CAM_PARAMAS['intrinsics'][f'C{camera}']['distortion']
        val = [distortion[2], distortion[3]]
